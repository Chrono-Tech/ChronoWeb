...
document.addEventListener('DOMContentLoaded', function () {
	...
	var $job__modal = $("#job__modal"),
			$job__modal__dialog = $("#job__modal__dialog"),
			$bio__modal = $("#bio__modal"),
			$bio__modal__dialog = $("#bio__modal__dialog");

	$('.vacancies .button').click(function () {
		showModal($job__modal);
	});

	$('.employees__human_bio-link').click(function () {
		showModal($bio__modal);
	});

	$('#job-close__cross').click(function () {
		hideModal()
	});

	function showModal($modal) {
		if (!$modal.hasClass("show")) {
			var m_top = Math.round(($(window).height() - $job__modal__dialog.outerHeight()) / 2);
			if (m_top > 45) {
				$job__modal__dialog.css({marginTop: m_top});
			}
			$job__modal.addClass("show");
		}
	}
